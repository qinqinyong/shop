        ...
        <table class="order-item" v-for="(order, index) in myOrderList" :key='order.id'>
          ...
          <tbody>
            ...
            <tr v-for="(orderDetail, index) in order.orderDetailList" :key='orderDetail.id'>
              ...
              <!-- 剩余4列  合并除第一列的单元格:rowspan="order.orderDetailList.length" 
                合并后用<template v-if="index === 0"> 包裹这4列，只展示第一行数据的列，下行的数据的列会从第一行拉取下来  -->
...
export default {
  ...
  methods: {
    async getMyOrderInfo(page = 1) {
      this.page = page
      // page和limit是一上来就要用的参数，但是此时我们是没有的，需我们自己指定
      const result = await this.$API.myOrderInfo(this.page, this.limit)
      if (result.code === 200) {
