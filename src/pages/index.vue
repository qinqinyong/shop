  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="detail">
        ...
        <ul class="list clearFix" v-for="(goods, index) in detailArrayList" :key='goods.skuId'>
          <li>
            <img :src="goods.imgUrl" alt="" style="width: 100px;heighe:70px">
          </li>
          <li>
            <p>{{ goods.skuName }}</p>
            <h4>7天无理由退货</h4>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{ finallyAddress.userAddress }}</span>
    ...
    <div class="sub clearFix">
      ...
      <a class="subBtn" href="javasceipt:;" @click="submitOrder">提交订单</a>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async submitOrder() {
      ...
      try {
        const result = await this.$API.reqSubmitOrder(traderNo, tradeInfo)
        if (result.code === 200) { //result.data为后台返回数据中的订单编号
          this.$router.push('/pay?orderId='+result.data)  //路径携带订单号去下一个界面
        }
      } catch (error) {
        alert('请求失败')
      }
